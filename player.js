...
function setupSong(songData) {
    ...
    if (song.audio_url) {
        audioEl.src = `http://localhost:8080${song.audio_url}`;
    }
    ...
    for (let i = 0; i < song.lines.length; i++) {
        ...
        const end = i + 1 < song.lines.length ? song.lines[i + 1] : timedWords.length;
    ...
    songLength = timedWords[timedWords.length - 1].time + 5;
    audioEl.load();
    audioEl.pause();
